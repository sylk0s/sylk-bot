...
pub trait CloudSync where for<'a> Self: Deserialize<'a> + Serialize + Unique + Sync + Send {
    ...
    async fn clsave<T>(&self, collection: &'static str) -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    async fn clrm<T>(&self, collection: &'static str) -> Result<(), Box<dyn Error + Send + Sync>> {
        let db = FirestoreDb::new(&config_env_var("PROJECT_ID")?).await?;   
        db.delete_by_id(collection, self.uuid().to_string()).await?;
        Ok(())
    }
    ...
    async fn clget<T: for<'a> Deserialize<'a>>() ->  Result<Vec<T>, Box<dyn Error + Send + Sync>> {
        let db = FirestoreDb::new(&config_env_var("PROJECT_ID")?).await?;
